...
def runBenchmark(scenario: str, function: Callable, output_file: Optional[str] = None):
    ...
    if output_file:
        settings["output_data_file"] = os.path.join(
            f"{testing_directory}", "outputfiles", f"{output_file}.csv"
        )
    else:
        settings["output_data_file"] = os.path.join(
            f"{testing_directory}", "outputfiles", f"{scenario.split('.')[0]}.csv"
        )
    ...
    for filename, function, bname in zip(setting_files, functions, benchmark_names):
        for setting, name in zip(additional_settings, names):
            ...
            for j in range(n_runs):
                ...
                settings["log_file"] = os.path.join(
                    f"{testing_directory}", "logs", f"log_{bname}_{name}_{j}.log"
                )
                settings["output_data_file"] = os.path.join(
                    f"{testing_directory}", "outputfiles", f"out_{bname}_{name}_{j}.csv"
                )
                hypermapper.optimize(settings, function)
    ...
    for filename, function, bname in zip(setting_files, functions, benchmark_names):
        ...
        for setting, name in zip(additional_settings, names):
            ...
            for run in range(n_runs):
                ...
                with open(
                    os.path.join(
                        f"{testing_directory}", "logs", f"log_{bname}_{name}_{run}.log"
                    )
                ) as f:
                    yes = False
                    for line in f:
                        ...
                        if "f*" in line and yes:
                            ...
                            sigma.append(
                                float(line.split("sigma:")[-1].split("feasibility")[0])
                            )
                            yes = False
                fss.append(fs)
                mus.append(mu)
...
def performance_test():
    ...
    run_performance_test(
        setting_files, functions, benchmark_names, additional_settings, names, 1
    )
...
def rf_test(plot=False):
    ...
    additional_settings = [
        ...
        },
        {
            "models": {
                ...
                "bootstrap": "True",
                "use_all_data_to_fit_mean": "True",
                "use_all_data_to_fit_variance": "True",
            }
        },
        {
            "models": {
    ...
    if plot:
        plot_test(
            setting_files, functions, benchmark_names, additional_settings, names, 5
        )
    else:
        run_performance_test(
            setting_files, functions, benchmark_names, additional_settings, names, 10
        )
...
def crash_test():
    ...
    settings = read_settings_file(settings_file)
    settings["log_file"] = os.path.join(
        f"{testing_directory}", "logs", "branin4_scenario_gp_rs.log"
    )
    settings["output_data_file"] = os.path.join(
        f"{testing_directory}", "outputfiles", "branin4_scenario_gp_rs.csv"
    )
